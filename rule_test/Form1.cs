...
namespace rule_test
{
    public partial class Form1 : Form
    {
        ...
        static string add_pw = "48534A003D00006267AC0572E7980001011702020384000C02000003843132333435360000000000006267ABFB62678A60644F39400000000000FF";
        ...
        static string stop_card = "48534A002500006267D69A2EE7803001011000000384000C030000000384006267D68B";
        static string enable_card = "48534A002500006267D6F105E6C82101011000000384000C030000000384016267D6D0";
        static string pw_time = "48534A003000006267D32B34EB383401011A00000384000D0203846267D32B0162678A60644F3940000000000000";
        public Form1()
        {
            InitializeComponent();
        }
        ...
        {
            string rule_open;
            string crc_open;//存放两位crc校验
            string rule_open_f;
            byte[] buffer_open = new byte[lock_open.Length/2];//新建一个byte数组用来保存，长度是字符串长度的一半
            byte[] buffer_open1 = new byte[(lock_open.Length/2)+2];//保存完整命令帧，包括crc校验
...
//改变密码
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                rule_open = rule_open.Remove(48, 12);
                rule_open = rule_open.Insert(48, StringToHexString(textBox8.Text));
            }
            for (int i = 0; i < rule_open.Length; i += 2)//循环遍历字符串来赋值给byte数组
            {
                buffer_open[i / 2] = (byte)Convert.ToByte(rule_open.Substring(i, 2), 16);//按2位来截取转换为byte数据
...
//禁用钥匙
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (textBox10.Text.Length == 10&&textBox11.Text.Length==10)
            {
                int  datatime;
                string datatime_16;
                datatime=int.Parse(textBox10.Text);
                datatime_16 = datatime.ToString("x8");

                rule_open = rule_open.Remove(64, 8);
                rule_open = rule_open.Insert(64, datatime_16);

                datatime = int.Parse(textBox11.Text);
                datatime_16 = datatime.ToString("x8");
                rule_open = rule_open.Remove(72, 8);
                rule_open = rule_open.Insert(72, datatime_16);
            }
